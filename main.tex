...
\begin{compactenum}[\color{red} 1.]
  \item Log-Likelihood: $L(\mathbf{U}, \mathbf{V}) = \sum_{i,j} x_{i,j}\log p(w_j|d_i) = \break \sum_{(i,j) \in X} \log \sum_{z=1}^K p(w_j|z)p(z|d_i) =  \sum_{(i,j) \in X} \log \sum_{z=1}^K v_{zj}u_{zi} \break$ s.t. $\sum_{z} u_{z,i} = 1 \hspace{10pt}\&\hspace{10pt} \sum_{j} v_{z,j} = 1$
	...
	\item M-Steps: $u_{zi} = p(z|d_i) = \frac{\sum_j x_{ij}q_{zij}}{\sum_j x_{ij}}\hspace{10pt}\&\hspace{10pt} v_{zj} = p(w_j|z) = \frac{\sum_i x_{ij}q_{zij}}{\sum_{i,l}x_{il}q_{zil}}$
\end{compactenum}
...
\item[Act. func.] $ReLU(F(\mathbf{x};\mathbf{w}))_{n,m} = max(0, F_{n,m}(\mathbf{x};\mathbf{w}))$
...
$\mathbf{U} \in \mathbb{R}^{D \times  L}$ for \# atoms $ = L > D = \mathsf{dim}\text{(data)}$. Can't invert $U^T$, decoding $\rightarrow$ add constraint $\mathbf{z}^\star \in \argmin_\mathbf{z} \lVert \mathbf{z} \rVert_0$ s.t. $\mathbf{x} = \mathbf{Uz}$. NP-hard $\rightarrow$ approximate with 1-norm (convex) or with M. Pursuit.
