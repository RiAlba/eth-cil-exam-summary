...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item $\|\mathbf{x}\|_2 = \sqrt{\sum_{i=1}^{N} \mathbf{x}_i^2} = \sqrt{\langle \mathbf{x}, \mathbf{x} \rangle}$ \break
	\item $\mathbf{M} \in \mathbb{R}^{m \times n}$, $\|\mathbf{M}\|_1 = \sum_{i,j} | m_{i,j}|$
	\item $\|\mathbf{M}\|_2 = \sigma_{\text{max}}(\mathbf{M})$
	\item $\|\mathbf{M}\|_F =\allowbreak \sqrt{\sum_{i=1}^{m} \sum_{j=1}^{n}\mathbf{m}_{i,j}^2} =\allowbreak \sqrt{\sum_{i=1}^{\min\{m, n\}} \sigma_i^2} = \sqrt{trace(MM^T)}$
...
\begin{inparaitem}[\color{red}\textbullet]
	\item Marginal dist: $P(x) := Pr[X = x] := \sum_{y \in Y} P(x, y)$
	\item Conditional dist: $P(x|y) := Pr[X = x | Y = y] := \frac{P(x,y)}{P(y)},\quad \text{if } P(y) > 0$
	\item $\forall y \in Y: \sum_{x \in X} P(x|y) = 1$
	\item $P(x, y) = P(x|y) P(y)$
	\item \textbf{Bayes' rule}: $P(x|y) = \frac{P(y|x)P(x)}{P(y)}$
	\item $P(x|y) = P(x)$ (if $X$, $Y$ independent)
	\item $P(x_1, \ldots, x_n) = P(x_1 \cap \ldots \cap x_n) = \prod_{i=1}^n P(x_i)$ (if IID)
	\item $P(x_1 \cup x_2) = P(x_1) + P(x_2) - P(x_1,x_2)$
\end{inparaitem}
...
Using only $K$ largest eigenvalues and corresponding eigenvectors. $\tilde{\mathbf{A}}_{i, j} = \sum_{k}^K \mathbf{U}_{i, k} \mathbf{D}_{k,k} \mathbf{V}_{j, k} = \sum_{k}^K \mathbf{U}_{i, k} \mathbf{D}_{k,k} (\mathbf{V}^\top)_{k, j}$.\\
...
\begin{inparaenum}[\color{red} 1.]
	...
	\item Repeat from step 2, stops if termination condition is met (e.g. $\mathbf{Z} == \mathbf{Z}^\text{new}$).
\end{inparaenum}
...
\begin{compactdesc}
	...
	\item[Latent variable (generative model):] $\\z_k \in \{0, 1\}$, $\sum_{k=1}^K z_k = 1$, $\operatorname{Pr}(z_k = 1) = \pi_k \Leftrightarrow p(\mathbf{z}) = \prod_{k=1}^K \pi_k^{z_k}$
...
\begin{compactenum}[\color{red} 1.]
	...
	\item If termination condition (e.g. $\| \log p(\mathbf{X} | \boldsymbol{\theta}^{(t)}) - \log p(\mathbf{X} | \boldsymbol{\theta}^{(t-1)}) \| < \epsilon$) is not met, $t=t+1$ and go to step
	2.
\end{compactenum}
...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item BIC penalizes complexity more. The smaller the better. 
\end{inparaitem}
...
\begin{compactdesc}
	...
	\item[Symmetr. parameterization:] $p(w, d) = \sum_z p(z)p(w | z) p(d | z)$
\end{compactdesc}

\subsection{EM for pLSA:}
\begin{compactenum}[\color{red} 1.]
  \item Log-Likelihood: $L(\mathbf{U}, \mathbf{V}) = \sum_{i,j} x_{i,j}\log p(w_j|d_i) = \break \sum_{(i,j) \in X} \log \sum_{z=1}^K p(w_j|z)p(z|d_i) =  \sum_{(i,j) \in X} \log \sum_{z=1}^K v_{zj}u_{zi} \break$ s.t. $\sum_{z} u_{z,i} = 1 \hspace{10pt}\&\hspace{10pt} \sum_{j} v_{z,j} = 1$
	\item E-Step: $q_{zij} = \frac{p(w_j|z)p(z|d_i)}{\sum_{k=1}^K p(w_j|k)p(k|d_i)} = \frac{v_{zj}u_{zi}}{\sum_{k=1}^K v_{kj}u_{ki}}$
	\item M-Step: $u_{zi} = p(z|d_i) = \frac{\sum_j x_{ij}q_{zij}}{\sum_j x_{ij}};~v_{zj} = p(w_j|z) = \frac{\sum_i x_{ij}q_{zij}}{\sum_{i,l}x_{il}q_{zil}}$
\end{compactenum}

\subsection{Projected ALS for quadratic cost function}
\begin{inparaitem}[\color{red}\textbullet]
...
$\min_{\mathbf{U}, \mathbf{V}} J(\mathbf{U}, \mathbf{V}) = \frac{1}{2} \|\mathbf{X} - \mathbf{U}^\top\mathbf{V}\|_F^2$ s.t. $u_{zi},v_{zj} \geq 0~(\forall i,j,z)$
...
\textbf{Neurons}: $F_\sigma(\mathbf{x};\mathbf{w}) = \sigma(w_0 + \sum_{i=1}^M{x_iw_i})$. \textbf{Output}: linear; $\mathbf{y} = \mathbf{W}^L\mathbf{x}^{L-1}$, logistic; $y_1 = \text{P}[Y=1|\mathbf{x}] = \frac{1}{1 + \exp[-\langle \mathbf{w}_1^L,\mathbf{x}^{L-1}\rangle]}$, soft-max; $y_k = \text{P}[Y=k|\mathbf{x}]= \frac{\exp[\langle \mathbf{w}_k^L,\mathbf{x}^{L-1}\rangle]}{\sum_{m=1}^{K}{\exp[\langle \mathbf{w}_m^L, \mathbf{x}^{L-1}\rangle]}}$. \textbf{Loss function} $l(y, \hat{y})$: squared loss; $\frac{1}{2}(y - \hat{y})^2$, cross-entropy loss; $-y \log \hat{y} - (1-y)\log(1-\hat{y})$.  \textbf{Backpropagation} $J_{ij}^l=\frac{\partial x_i^l}{\partial x_j^{l-1}}=w_{ij}\sigma'(w_i^Tx)$;$\frac{\partial x^l}{\partial x^{l-n}} = J^lJ^{l-1} \dotsm J^{l-n+1}$; $ \nabla_{x^l} l(y, \hat{y})^T = \nabla_{y} l(y, \hat{y})^TJ^lJ^{l-1} \dotsm J^{l-n+1}$;
\break$ \frac{\partial l(y, \hat{y})}{\partial w_{ij}^l} = \frac{\partial l(y, \hat{y})}{\partial x_{i}^l} \frac{\partial x_i^l}{\partial w_{ij}^l}$; $\frac{\partial x_i^l}{\partial w_{ij}^l} = \sigma'({w_i^l}^Tx^{l-1})x_j^{l-1}$

\subsection{Convolutional Neural Networks for Images}
$\mathbf{(n,m)}$: pixel coords. \hspace{5pt} $\mathbf{x}$: image \hspace{5pt} $\mathbf{w}$: weights \hspace{5pt} $\mathbf{c}$: channels
\begin{compactdesc}
\item[Convolution:] $F_{n,m}(\mathbf{x};\mathbf{w}) = \sigma(\sum_{1}^c{(b + \sum_{k,l=-2}^2{w_{k,l}x_{n+k,m+l}})})$ 
\item[Act. func:] $\sigma = ReLU(F(\mathbf{x};\mathbf{w}))_{n,m} = max(0, F_{n,m}(\mathbf{x};\mathbf{w}))$
\item[Pooling:] $max_{-2 < i,j <2}~(ReLU(F(\mathbf{x};\mathbf{w}))_{n+i,m+j})$
\end{compactdesc}
...
projection is unique if $Q$ convex.

\subsection{Lagrangian Multipliers}
Minimize  $f(\mathbf{x})$ s.t. $g_i(\mathbf{x}) \leq 0,\ i = 1, .., m$ (\textbf{inequality constr.}) and $h_j(\mathbf{x}) = 0,\ j = 1, .., p$ (\textbf{equality constraint})
\begin{compactdesc}
...
$f : \mathbb{R}^D \rightarrow \mathbb{R}$ is convex, if $epi\ f$ is a convex set, and if $\forall \mathbf{x}, \mathbf{y} \in dom\ f$, and for $0 \leq \alpha \leq 1$: $f(\alpha \mathbf{x} + (1 - \alpha)\mathbf{y}) \leq \alpha f(\mathbf{x}) + (1-\alpha)f(\mathbf{y})$. local=global min, \textbf{Convergence}: all descent methods converge; $f(\mathbf{x}^{(t)}) - f(\mathbf{x}^*) \le \frac{c}{t}$.

\section{Sparse Coding}

\subsection{Orthogonal Basis}
Sparse representation $\mathbf{z} = \mathbf{U}^\top \mathbf{x} $. Approx $ \mathbf{\hat{x}} = \mathbf{U\hat{z}}$, $\hat{z}_i = z_i$ if $ \lvert z_i \rvert > \epsilon$ else 0.
\textbf{Reconstruction Error}: $\|\mathbf{x}-\mathbf{\hat{x}}\|^2$ and sparsity of $\hat{z}$.
\textbf{Choice of basis depends on signal}: fourier for global, wavelet for local and PCA basis for given $\Sigma$.
\textbf{Haar Wavelets}: $\mathbf{U}=\mathbf{W}\mathbf{S}$, $\mathbf{\hat{x}} = z_1\mathbf{w}_0 + z_2\mathbf{w}(x) + z_3\mathbf{w}(2x) + z_4\mathbf{w}(2x-1)$

\subsection{Overcomplete Basis}
$\mathbf{U} \in \mathbb{R}^{D \times L}$ for \# atoms $ = L > D = \mathsf{dim}$. Can't invert $U$, decoding $\rightarrow$ add constraint $\mathbf{z}^\star \in \argmin_\mathbf{z} \lVert \mathbf{z} \rVert_0$ s.t. $\mathbf{x} = \mathbf{Uz}$. NP-hard $\rightarrow$ approximate with 1-norm (convex) or with M. Pursuit.

\textbf{Coherence}
\begin{inparaitem}[\color{red}\textbullet]
	\item $m(\mathbf{U}) = \max_{i,j:\, i \neq j} | \mathbf{u}_i^\top \mathbf{u}_j |$
	\item $m(\mathbf{B}) = 0$ if $\mathbf{B}$ orthogonal
	\item $m([\mathbf{B}, \mathbf{u}]) \geq \frac{1}{\sqrt{D}}$ if atom $\mathbf{u}$ is added to orthogonal basis $\mathbf{B}$.  To min coherence max angle btw. atoms. Rotate $\mathbf{x}$ in 2D, $\mathbf{x}_1' = \cos \theta ~\mathbf{x}_1 - \sin \theta ~\mathbf{x}_2$; $\mathbf{x}_2' = \sin \theta ~\mathbf{x}_1 + \cos \theta ~\mathbf{x}_2$
\end{inparaitem}
...
\subsection{Dictionary Learning (Find U and Z)}
...
\begin{inparaitem}[\color{red}\textbullet]
  ...
  \textbf{Updates}: $\mathbf{L}^{t+1} = argmin_L~L_p(L,S,\lambda) =  \mathcal{D}_{\rho^{-1}}(X-S-\rho^{-1}\text{mat}(\lambda))$; $\mathbf{S}^{t+1} = argmin_S~L_p(L,S,\lambda) =  \mathcal{S}_{\mu\rho^{-1}}(X-S-\rho^{-1}\text{mat}(\lambda))$; $\boldsymbol{\lambda}^{t+1} = \boldsymbol{\lambda}^t + \rho vec(L^{t+1} + S^{t+1} - X)$ \break
  $\mathcal{D}_\tau = U\mathcal{S}_t(\Sigma)V^T$;
  $\mathcal{S}_\tau(X) = sgn(x)max(x - \tau, 0)$
\end{inparaitem}
